// Vercel serverless function to steal a gift
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (gift) {
      ...
      await pusher.trigger('game-channel', 'gift-stolen', {
        gift,
      ...
      return res.status(400).json({ error: 'Cannot steal that gift' });
    }
  } catch (error) {
    console.error('Error stealing gift:', error);
    return res.status(500).json({ error: error.message || 'Failed to steal gift' });
  }
}
